...
public class ProductController {
    ...
    @GetMapping("/products")
    public String products(@RequestParam(required = false) Long categoryId, @RequestParam(required = false) BigDecimal minPrice, @RequestParam(required = false) BigDecimal maxPrice, @RequestParam(required = false) String availability, @RequestParam(required = false) String brand, @RequestParam(required = false) String vmodel, @RequestParam(required = false) Integer year, @RequestParam(required = false) String modification, Model model) {
        ...
        model.addAttribute("products", productService.getFilteredProducts(categoryId, minPrice, maxPrice, availBool, brand, vmodel, year, modification));
    ...
    public String productsByCategory(@PathVariable Long id, Model model) {
        model.addAttribute("categories", productService.getAllCategories());
        model.addAttribute("vehicles", productService.getAllVehicles());
        model.addAttribute("products", productService.getFilteredProducts(id, null, null, null, null, null, null, null));
    ...
    public String productDetail(@PathVariable Long id, Model model, HttpSession session) {
        ...
        List<Review> reviews = reviewService.getTopLevelReviews(course.project.ua.tirevault.Entities.Enums.ReviewTargetType.PRODUCT, id);
